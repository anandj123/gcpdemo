...
void calculate(int T) {
    int n; cin>>n;
    int ans =1;
    vi water(n); REP(i,n) cin >> water[i];
    bool prev_water = false;
    debug() << imie(water);

    REP(i,n){
      if (water[i] == 1 and prev_water){
        prev_water = true;
        ans+=5;
        continue;
      }
      if (water[i]==1 and prev_water==false) {
        prev_water = true;
        ans++;
        continue;
      }
      if (water[i]==0 and prev_water==false){
        prev_water = false;
        if (i==0) continue;
        cout << "-1" << endl;
        return;
      }
      if (water[i]==0 and prev_water==true){
        prev_water = false;
        continue;
      }
    }
